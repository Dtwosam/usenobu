...
- Paid `POST /v1/agent/monitoring-pass` (service **35958**, registered `https://www.usenobu.xyz/v1/agent/monitoring-pass`) — `$0.99` x402 v2 exact on X Layer (`eip155:196`) USD₮0 amount `990000`; unpaid **402**; official OKX seller **verify → settle → settle/status**; exactly one Monitoring Pass per settlement; **exactly one Purchase Setup journey ensured at settlement** (`ensureMarketplacePurchaseJourney`). If journey ensure fails after pass+continuation, response is `MONITORING_PASS_DELIVERY_PENDING` (payment preserved); `reconcilePendingPassSettlements` discovers missing journeys via `listSettledMonitoringPassPaymentsMissingJourney` (independent of pending/orphan/missing-continuation batches) and backfills one journey at `confirm_use_pass` without payment replay. New continuations store **no** claim hash; historical claim-hash recovery remains. Successful paid body returns the durable journey stage (first: `confirm_use_pass`) with neutral `protocol_continuation` carrying only public `journey_id` — **no** `pass_claim_credential` on newly issued passes.

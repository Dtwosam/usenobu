...
5. Ask the seven questions.
6. Thank the tester and stop.
...
|---|---|---|
| 1 | After viewing the homepage, explain what Nobu does in your own words. | Mentions post-purchase price watching / alerts after buy |
| 2 | Explain how Nobu may help someone get money back. | Mentions price drop + requesting the difference (not automatic refund) |
| 3 | Identify which purchases are currently supported. | Eligible Target.com (not “every store”) |
| 4 | Add and confirm a purchase, then find the monitoring status. | Reaches watching / Monitoring Proof |
| 5 | Explain what you would do if Nobu found a possible price difference. | Check Target / contact retailer / use next steps (not “Nobu refunds me”) |
...
Copy one block per tester into `docs/proof/user-testing/final-validation/` (real sessions only).

```
Tester code:
Date:
Time to first accurate product description (seconds):
Money-back benefit understood: yes / no / partial
Understood not guaranteed: yes / no / partial
Broader purpose understood: yes / no / partial
Current Target-only clear: yes / no / partial
Tasks completed (1–5):
Main confusion:
Suggested change:
Would use: yes / no / maybe
Quote permission: yes / no
Optional anonymous quote:
```

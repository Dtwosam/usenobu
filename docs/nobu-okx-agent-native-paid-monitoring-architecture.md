...
**Status (as of Lane 8R PASS):** Free agent actions, monitor management, durable scheduler bridge, email alerts, paid start-monitoring route, and official OKX seller verify/settle/status are **IMPLEMENTED and DEPLOYED** on UseNobu. ASP **`#5541` paid service is registered** (separate A2MCP fee `0.99` alongside free fee `0`); marketplace listing remains **under review** (not publicly listed). Genuine payment marketplace proof remains **Lane 7.4G**. Topology (§5) is RESOLVED (separate free + paid services under `#5541`). The free OpenAPI is `openapi/nobu-a2mcp.openapi.yaml` (all free actions). The paid OpenAPI file retains its historical filename but describes the implemented/deployed paid surface.

**Historical note:** Sections below may still describe intermediate states from Lanes 7.4A–7.4D research (e.g. “not deployed”, `not_configured`). Treat the status line above and `docs/nobu-architecture.md` / `docs/nobu-current-state.md` as current production truth.
...
- **Lane 8R — Accurate edit/resubmit of ASP #5541. COMPLETE (`NOBU_LANE_8R_PASS`).** Free service preserved; paid `$0.99` service created under `#5541`; production 402 proven; listing under review. Evidence: `docs/proof/lane-8r-asp-update/`. Next: Lane 7.4G when OKX.AI access is official.

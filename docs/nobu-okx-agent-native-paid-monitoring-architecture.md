...
**Status (superseded summary as of Lanes 8R.0–8R.2):** Free agent actions, monitor management, durable scheduler bridge, email alerts, paid start-monitoring route, and official OKX seller verify/settle/status are **IMPLEMENTED and DEPLOYED** on UseNobu. ASP **paid-service registration** remains **Lane 8R**. Genuine payment marketplace proof remains **Lane 7.4G**. Topology (§5) is RESOLVED (separate free + paid services under `#5541`). The free OpenAPI is `openapi/nobu-a2mcp.openapi.yaml` (all free actions). The paid OpenAPI file retains its historical filename but describes the implemented/deployed paid surface.
